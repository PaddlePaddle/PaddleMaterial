...
class OutputBlock(AtomUpdateBlock):
    ...
    ----------
        ...
        num_targets: int
            Number of targets.
        activation: str
            Activation function to use in the dense layers (except for the
            final dense layer).
        direct_forces: bool
            If true directly predict forces without taking the gradient of
    ...
    def reset_parameters(self):
        if self.output_init.lower() == "heorthogonal":
            ...
            init_Constant(self.out_energy.weight)
            if self.direct_forces:
                init_Constant = paddle.nn.initializer.Constant(value=0.0)
                init_Constant(self.out_forces.weight)
        else:

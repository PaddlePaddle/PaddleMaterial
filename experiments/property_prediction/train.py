...
def main(config: DictConfig):
    ...
    # Build config of Metric
    metric_keys = config["_Metric"].get("keys")
    metric_values = config["_Metric"].get("values")
    if len(metric_keys) != len(metric_values):
        raise ValueError(
            "The number of metric keys does not match the number of metric values."
        )
    config["Metric"] = dict(zip(metric_keys, metric_values))

...
# -*- coding: utf-8 -*-
"""
SevenNet Interatomic Potential Training Script
独立训练脚本，不依赖项目主框架
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train SevenNet interatomic potential")
...
def main():
    ...
    # 创建输出目录
    save_dir = args.output_dir or config["log"].get("save_dir", "./output/sevennet")
    ...
    if not os.path.exists(data_path):
        print(f"Warning: Dataset not found at {data_path}, using built-in test data")
        data_path = os.path.join(PROJECT_ROOT, "interatomic_potentials/example_data", "hfo2.extxyz")

...
class TestWDMPNNForwardAlignment(unittest.TestCase):
    ...
    def test_reference_alignment(self):
        """Test alignment with pre-computed reference .npz values.

        Reference values were generated by raw_infer_data/create_input_output_npz.py
        with seed=42 and saved to reference_io.npz.  This ensures the model
        doesn't silently change behavior across refactors.
        """
        npz_path = os.path.join(
            os.path.dirname(__file__), os.pardir, "raw_infer_data", "reference_io.npz"
        )
        self.assertTrue(os.path.exists(npz_path), f"Reference file missing: {npz_path}")
        ...
        max_pred_diff = np.abs(ref["pred"] - pred).max()
        max_loss_diff = np.abs(ref["loss"] - loss).max()
        self.assertLess(max_pred_diff, 1e-4, f"Prediction drift: {max_pred_diff}")
        self.assertLess(max_loss_diff, 1e-4, f"Loss drift: {max_loss_diff}")
    ...
    def test_training_convergence(self):
        ...
        data = self._create_batch_data(n_mols=2, label_val=5.0)
        losses = []

...
class ASUVESDEScheduler(nn.Layer):
    ...
    def _sigma_norm_asu_wrapped(
        ...
        logger.info(f"Checking cache: {cache_filename}")
        logger.info(f"Cache exists: {os.path.exists(cache_filename)}")
        if os.path.exists(cache_filename):
            logger.info(f"Loading sigma norms from: {cache_filename}")
        ...
        logger.info(f"Saved sigma norms to: {cache_filename}")
        return sigma_norms

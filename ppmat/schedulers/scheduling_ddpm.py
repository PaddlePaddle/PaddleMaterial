...
class DDPMSchedulerOutput:
    ...
    Args:
        ...
        pred_original_sample (`paddle.Tensor` of shape `(batch_size, num_channels,
            height, width)` for images): The predicted denoised sample `(x_{0})` based
            on the model output from the current timestep. `pred_original_sample` can
...
def betas_for_alpha_bar(
    ...
    Args:
        ...
        alpha_transform_type (`str`, *optional*, default to `cosine`): the type of
        noise schedule for alpha_bar. Choose from `cosine` or `exp`
...
class DDPMScheduler:
    """
    `DDPMScheduler` explores the connections between denoising score matching and
    ...
    Args:
        ...
        beta_schedule (`str`, defaults to `"linear"`):
            The beta schedule, a mapping from a beta range to a sequence of betas for
            stepping the model. Choose from `linear`, `scaled_linear`, or
            `squaredcos_cap_v2`.
        variance_type (`str`, defaults to `"fixed_small"`):
            Clip the variance when adding noise to the denoised sample. Choose from
            `fixed_small`, `fixed_small_log`, `fixed_large`, `fixed_large_log`,
            `learned` or `learned_range`.
        ...
        prediction_type (`str`, defaults to `epsilon`, *optional*):
            Prediction type of the scheduler function; can be `epsilon` (predicts the
            noise of the diffusion process), `sample` (directly predicts the noisy
            sample`) or `v_prediction` (see section 2.4 of [Imagen
            Video](https://imagen.research.google/video/paper.pdf) paper).
        thresholding (`bool`, defaults to `False`):
            Whether to use the "dynamic thresholding" method. This is unsuitable for
        ...
        steps_offset (`int`, defaults to 0):
            ...
            `offset=1` and `set_alpha_to_one=False` to make the last step use step 0
            for the previous alpha product like in Stable Diffusion.
        rescale_betas_zero_snr (`bool`, defaults to `False`):
            Whether to rescale the betas to have zero terminal SNR. This enables
            the model to generate very bright and dark samples instead of limiting it
            to samples with medium brightness.
    """
    ...
    def __init__(
        ...
        if trained_betas is not None:
            ...
            self.betas = (
                paddle.nn.functional.sigmoid(betas) * (beta_end - beta_start)
                + beta_start
            )
        else:
    ...
    def set_timesteps(
        ...
        """
        Sets the discrete timesteps used for the diffusion chain (to be run before
        inference).
        ...
        Args:
            ...
            timesteps (`List[int]`, *optional*):
                ...
                between timesteps is used. If `timesteps` is passed,
        ...
        if timesteps is not None:
            ...
            if num_inference_steps > self.num_train_timesteps:
                raise ValueError(
                    ...
                    f"than `self.train_timesteps`: {self.num_train_timesteps} as the "
                    "unet model trained with this scheduler can only handle"
            ...
            if self.timestep_spacing == "linspace":
                ...
                raise ValueError(
                    f"{self.timestep_spacing} is not supported. Please make sure to "
    ...
    def _threshold_sample(self, sample: paddle.Tensor) -> paddle.Tensor:
        ...
        absolute pixel value in xt0 (the prediction of x_0 at timestep t), and if
        s > 1, then we threshold xt0 to the range [-s, s] and then divide by s.
        Dynamic thresholding pushes saturated pixels (those near -1 and 1) inwards,
        thereby actively preventing pixels from saturation at each step. We find that
        dynamic thresholding results in significantly better photorealism as well as
        ...
        if self.sample_max_value < 1:
            s = paddle.ones_like(s) * self.sample_max_value
        else:
            s = paddle.clip(
    ...
    def step(
        ...
        Returns:
            [`~schedulers.scheduling_ddpm.DDPMSchedulerOutput`] or `tuple`:
                ...
                [`~schedulers.scheduling_ddpm.DDPMSchedulerOutput`] is returned,
                otherwise a tuple is returned where the first element is the sample
                tensor.
        ...
        if self.prediction_type == "epsilon":
            ...
            raise ValueError(
                f"prediction_type given as {self.prediction_type} must be one of "
                "`epsilon`, `sample` or `v_prediction`  for the DDPMScheduler."
            )
    ...
    def get_velocity(
        self, sample: paddle.Tensor, noise: paddle.Tensor, timesteps: paddle.Tensor
    ) -> paddle.Tensor:

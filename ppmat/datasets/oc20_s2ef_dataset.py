...
# -----------------------------------------------------------------------------
_OC20_BASE_URL = (
    "https://paddle-org.bj.bcebos.com/paddlematerials/datasets/OC20/s2ef_train_2M"
)

OC20_S2EF_TRAIN_2M_URLS = [
    f"{_OC20_BASE_URL}/0000.parquet",
    f"{_OC20_BASE_URL}/0001.parquet",
    f"{_OC20_BASE_URL}/0002.parquet",
    f"{_OC20_BASE_URL}/0003.parquet",
    f"{_OC20_BASE_URL}/0004.parquet",
    f"{_OC20_BASE_URL}/0005.parquet",
    f"{_OC20_BASE_URL}/0006.parquet",
    f"{_OC20_BASE_URL}/0007.parquet",
    f"{_OC20_BASE_URL}/0008.parquet",
    f"{_OC20_BASE_URL}/0009.parquet",
    f"{_OC20_BASE_URL}/0010.parquet",
    f"{_OC20_BASE_URL}/0011.parquet",
    f"{_OC20_BASE_URL}/0012.parquet",
    f"{_OC20_BASE_URL}/0013.parquet",
]
...
class OC20S2EFDataset(Dataset):
    ...
    **Directory Structure**
    ...
    def _prepare_structures_and_properties(self, local_shards):
        """
        Check if structures/properties are cached; rebuild if missing or overwrite=True.
        """
    ...
    def _prepare_graphs(self):
        """
        Check if graphs are cached; rebuild if missing/incomplete/overwrite=True.
        """
    ...
    def _build_graphs(self, converter, structures_dir: str, graphs_dir: str) -> None:
        ...
        # Context manager to suppress stderr (used to mute internal tqdm of libraries)
    ...
    def _ensure_length_consistency(self):
        """
        Ensures consistency in length across structures, graphs, and all property arrays
        Truncates data to the minimum length found to prevent index errors.
        """
    ...
    def _ensure_shards(self) -> List[str]:
        """
        Ensures all shard files are present locally. Downloads missing shards from URLs.
        """
        ...
        **models trained on this synthesized data will be physically meaningless**
        ...
        for shard in shard_paths:
            ...
            for rg in range(pf.num_row_groups):
                ...
                # Since 'pos' or 'atoms' might missing, check other columns.
                nrows = 0
    ...
    def _filter_by_properties(self) -> None:
        ...
        if len(keep) < total:
            logger.warning(
                f"Filtering: Dropping {total - len(keep)} samples due to "
                "invalid properties."
            )
    ...
    def _filter_by_graphs(self) -> None:
        ...
        Since graphs are rebuilt fully if mismatch occurs, this is mostly a sanity check
        """
        pass

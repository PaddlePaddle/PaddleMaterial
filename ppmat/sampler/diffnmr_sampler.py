...
DIFFNMR_VOCAB_URL = (
    "https://paddle-org.bj.bcebos.com/paddlematerials/assets/vocabs/"
    "msd_nmr_vocab.zip"
)
DIFFNMR_RETRIEVAL_URLS = {
    "msd_nmr_nless15_retrieval_molecular_representations.csv": (
        "https://paddle-org.bj.bcebos.com/paddlematerials/assets/databases/"
        "msd_nmr_nless15_retrieval_molecular_representations.zip"
    ),
    "msd_nmr_nless20_retrieval_molecular_representations.csv": (
        "https://paddle-org.bj.bcebos.com/paddlematerials/assets/databases/"
        "msd_nmr_nless20_retrieval_molecular_representations.zip"
    ),
}
...
class DiffNMRSampler:
    ...
    def _resolve_package_paths(
        ...
        vocab_asset_root = None
        retrieval_asset_roots = {}

        def resolve_vocab_asset(path):
            nonlocal vocab_asset_root
            if not (
                path.startswith("./assets/vocab/") or path.startswith("assets/vocab/")
            ):
                return None
            if vocab_asset_root is None:
                vocab_asset_root = download.get_assets_path_from_url(DIFFNMR_VOCAB_URL)
            rel_path = path.split("assets/vocab/", 1)[1]
            candidate = osp.join(vocab_asset_root, rel_path)
            if osp.exists(candidate):
                return candidate
            return None

        def resolve_retrieval_asset(path):
            if not (
                path.startswith("./assets/retrival_database/")
                or path.startswith("assets/retrival_database/")
            ):
                return None
            file_name = osp.basename(path)
            if file_name not in DIFFNMR_RETRIEVAL_URLS:
                return None
            if file_name not in retrieval_asset_roots:
                retrieval_asset_roots[file_name] = download.get_assets_path_from_url(
                    DIFFNMR_RETRIEVAL_URLS[file_name]
                )
            rel_path = path.split("assets/", 1)[1]
            candidate = osp.join(retrieval_asset_roots[file_name], rel_path)
            if osp.exists(candidate):
                return candidate
            return None
        ...
        def resolve_path(path):
            ...
            candidate = resolve_vocab_asset(path)
            if candidate is not None:
                return candidate

            candidate = resolve_retrieval_asset(path)
            if candidate is not None:
                return candidate

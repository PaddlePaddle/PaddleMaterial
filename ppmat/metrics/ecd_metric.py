...
class ECDMetrics(nn.Layer):
    """Evaluation metrics for ECFormer ECD task.
    
    Computes:
        - Number-RMSE: RMSE of predicted vs true peak count
        - Position-RMSE: RMSE of predicted vs true peak positions (class indices)
        - Symbol-Acc: Accuracy of predicted peak symbols (positive/negative)
        - First-Symbol-Acc: Accuracy of the first peak's symbol
    """
    
    def __init__(self, num_position_classes=20, max_peaks=9):
        """
        Args:
            num_position_classes (int): Number of position classes (default: 20)
            max_peaks (int): Maximum number of peaks (default: 9)
        """
        super().__init__()
    ...
    def reset(self):
        ...
        self.symbol_correct = _to_f32(0.0)
        self.symbol_total = _to_f32(0.0)
        self.first_symbol_correct = _to_f32(0.0)
        self.first_symbol_total = _to_f32(0.0)
    ...
    def update(self, predictions: Dict[str, paddle.Tensor], targets: Dict[str, paddle.Tensor]):
        ...
        # Process each sample for position and symbol metrics
        for i in range(batch_size):
            ...
            if n_true > 0 and n_pred > 0:
                ...
                # Symbol accuracy
                height_true = targets['peak_height'][i, :n_match]
                height_pred = predictions['peak_height'][i, :n_match, :].argmax(axis=1)
                correct = (height_true == height_pred).astype('float32')
                self.symbol_correct += paddle.sum(correct)
                self.symbol_total += _to_f32(n_match)
                
                # First peak symbol accuracy
                if height_true[0] == height_pred[0]:
                    self.first_symbol_correct += _to_f32(1.0)
                self.first_symbol_total += _to_f32(1.0)
    ...
    def accumulate(self) -> Dict[str, float]:
        ...
        symbol_correct = _all_reduce_sum_(self.symbol_correct.clone())
        symbol_total = _all_reduce_sum_(self.symbol_total.clone())
        first_correct = _all_reduce_sum_(self.first_symbol_correct.clone())
        first_total = _all_reduce_sum_(self.first_symbol_total.clone())
        ...
        # For position RMSE, we need to average over matched peaks
        pos_rmse = paddle.sqrt(
            pos_rmse_sum / paddle.maximum(symbol_total, _to_f32(1.0))
        ).item()
        
        symbol_acc = (symbol_correct / paddle.maximum(symbol_total, _to_f32(1.0))).item()
        first_symbol_acc = (first_correct / paddle.maximum(first_total, _to_f32(1.0))).item()
...
class ECFormerIRMetrics(nn.Layer):
    """Evaluation metrics for ECFormer IR task.
    
    Computes:
        - Number-RMSE: RMSE of predicted vs true peak count
        - Position-RMSE: RMSE of predicted vs true peak positions (class indices)
        - Height-RMSE: RMSE of predicted vs true peak intensities
    """
    
    def __init__(self, num_position_classes=36, max_peaks=15, use_height_prediction=True):
        """
        Args:
            num_position_classes (int): Number of position classes (default: 36 for IR)
            max_peaks (int): Maximum number of peaks (default: 15)

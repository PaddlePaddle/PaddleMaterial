...
class StructureSampler:
    ...
    def compute_metric(
        ...
        total_results = self.sample_by_dataloader(self.save_path)
            ...
            if self.save_path is not None:
                ...
                for i, structure in enumerate(structures):
                    ...
                    tar_file = os.path.join(
                        self.save_path, f"{formula}_{iter_id + 1}_{i + 1}.cif"
                    )
    ...
    def sample_by_num_atoms(self, num_atoms, sample_params=None):
        ...
        if self.save_path is not None:
            ...
            for i, structure in enumerate(structures):
                formula = structure.formula.replace(" ", "-")
                tar_file = os.path.join(self.save_path, f"{formula}_{i + 1}.cif")
                if structure is not None:
                    writer = CifWriter(structure)
                    writer.write_file(tar_file)
                else:
    ...
    def sample_by_chemical_formula(self, chemical_formula, sample_params=None):
    ...
    def sample_by_condition(self, composition, sample_params=None):

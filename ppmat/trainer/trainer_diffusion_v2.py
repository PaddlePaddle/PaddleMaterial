...
class DiscreteDenoisingDiffusionMolecular_condition(nn.Layer):
    ...
    def forward(self, noisy_data, extra_data, node_mask, X, E):
        ...
        # 将去噪输入拼到一起
        X_ = paddle.concat(
            [noisy_data["X_t"], extra_data.X], axis=2
        )  # (bs, n, dX + extra)
        E_ = paddle.concat(
            [noisy_data["E_t"], extra_data.E], axis=3
        )  # (bs, n, n, dE + extra)
        y_ = paddle.concat(
            [noisy_data["y_t"], extra_data.y], axis=1
        )  # (bs, dy + extra)
    ...
    def apply_noise(self, X, E, y, node_mask):
        ...
        t_int = paddle.randint(
            lowest_t, self.T + 1, shape=[X.shape[0], 1], dtype="int64"
        ).astype(
            "float32"
        )  # (bs,1)
        s_int = t_int - 1
        ...
        Qtb = self.transition_model.get_Qt_bar(
            alpha_t_bar, device=None
        )  # 不显式 device, Paddle 中 tensor 会自动管理

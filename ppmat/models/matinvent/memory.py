...
def _comp_ele(strucs):
    return ([s.composition.reduced_formula for s in strucs],
            [tuple(sorted(set(str(e) for e in s.species))) for s in strucs])
...
class LongTimeMem:
    ...
    def extend(self, strucs: List[Structure], rewards: np.ndarray, step: int):
        comps, ele_comb = _comp_ele(strucs)
...
class ReplayBuffer:
    def __init__(self, buffer_size: int = 100, sample_size: int = 8,
                 reward_cutoff: float = 0.0, capacity: int = None):
        self.buffer_size = capacity or buffer_size
        self.sample_size = sample_size
        self.reward_cutoff = reward_cutoff
        self.buffer = pd.DataFrame(columns=["data", "struc", "comp", "ele_comb", "reward"])

    def extend(self, data: list, strucs: List[Structure], rewards: np.ndarray):
        comps, ele_comb = _comp_ele(strucs)

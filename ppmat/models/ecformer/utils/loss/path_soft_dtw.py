...
class PathDTWBatch(PyLayer):
    @staticmethod
    def forward(ctx, D, gamma): # D.shape: [batch_size, N , N]
        batch_size, N, N = D.shape
        device = D.place
        D_cpu = D.detach().cpu().numpy()
        gamma_paddle = paddle.to_tensor([gamma], dtype='float32').to(device)
        
        grad_paddle = paddle.zeros((batch_size, N ,N), place=device)
        Q_paddle = paddle.zeros((batch_size, N+2 ,N+2, 3), place=device)
        E_paddle = paddle.zeros((batch_size, N+2 ,N+2), place=device)  
        
        for k in range(0, batch_size): # loop over all D in the batch    
            _, grad_cpu_k, Q_cpu_k, E_cpu_k = dtw_grad(D_cpu[k,:,:], gamma)     
            grad_paddle[k,:,:] = paddle.to_tensor(grad_cpu_k, dtype='float32').to(device)
            Q_paddle[k,:,:,:] = paddle.to_tensor(Q_cpu_k, dtype='float32').to(device)
            E_paddle[k,:,:] = paddle.to_tensor(E_cpu_k, dtype='float32').to(device)
        
        ctx.save_for_backward(grad_paddle, D, Q_paddle, E_paddle, gamma_paddle) 
        return paddle.mean(grad_paddle, axis=0) 
    
    @staticmethod
    def backward(ctx, grad_output):
        device = grad_output.place
        grad_paddle, D_paddle, Q_paddle, E_paddle, gamma = ctx.saved_tensor()
        D_cpu = D_paddle.detach().cpu().numpy()
        Q_cpu = Q_paddle.detach().cpu().numpy()
        E_cpu = E_paddle.detach().cpu().numpy()
        gamma = gamma.detach().cpu().numpy()[0]
        Z = grad_output.detach().cpu().numpy()
        
        batch_size, N, N = D_cpu.shape
        Hessian = paddle.zeros((batch_size, N ,N), place=device)
        
        for k in range(0, batch_size):
            _, hess_k = dtw_hessian_prod(D_cpu[k,:,:], Z, Q_cpu[k,:,:,:], E_cpu[k,:,:], gamma)
            Hessian[k:k+1,:,:] = paddle.to_tensor(hess_k, dtype='float32').to(device)

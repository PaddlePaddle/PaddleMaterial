...
def permute_wigner_inv_edge_to_node_bwd_dx_kernel(
    ...
    while edge_id < num_edges:
        ...
        tl.store(DX_ptr + dx_base + 0 * sphere_channels + c_range, dx0, mask=c_mask)
        tl.store(DX_ptr + dx_base + 1 * sphere_channels + c_range, dx2, mask=c_mask)
        tl.store(DX_ptr + dx_base + 2 * sphere_channels + c_range, dx6, mask=c_mask)
        tl.store(DX_ptr + dx_base + 3 * sphere_channels + c_range, dx3, mask=c_mask)
        tl.store(DX_ptr + dx_base + 4 * sphere_channels + c_range, dx7, mask=c_mask)
        tl.store(DX_ptr + dx_base + 5 * sphere_channels + c_range, dx1, mask=c_mask)
        tl.store(DX_ptr + dx_base + 6 * sphere_channels + c_range, dx5, mask=c_mask)
        tl.store(DX_ptr + dx_base + 7 * sphere_channels + c_range, dx8, mask=c_mask)
        tl.store(DX_ptr + dx_base + 8 * sphere_channels + c_range, dx4, mask=c_mask)
        edge_id += GRID_E_STRIDE


@triton.jit
def permute_wigner_inv_edge_to_node_bwd_dw_kernel(
    DY_ptr, X_ptr, DW_ptr, num_edges, C: tl.constexpr, GRID_E_STRIDE: tl.constexpr
):
    """

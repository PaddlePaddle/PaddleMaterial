...
class CaseCoeffs(paddle.nn.Module):
    """
    ...
    def __init__(
        self,
        coeff: paddle.Tensor,
        horner: paddle.Tensor,
        poly_len: paddle.Tensor,
        ra_exp: paddle.Tensor,
        rb_exp: paddle.Tensor,
        sign: paddle.Tensor,
    ):
        super().__init__()
...
class WignerCoefficients(paddle.nn.Module):
    """
    ...
    def __init__(
        ...
        n_primary: int,
        n_derived: int,
        primary_row: paddle.Tensor,
        primary_col: paddle.Tensor,
        case1: CaseCoeffs,
        case2: CaseCoeffs,
        mp_plus_m: paddle.Tensor,
        m_minus_mp: paddle.Tensor,
        diagonal_mask: paddle.Tensor,
        anti_diagonal_mask: paddle.Tensor,
        special_2m: paddle.Tensor,
        anti_diag_sign: paddle.Tensor,
        derived_row: paddle.Tensor,
        derived_col: paddle.Tensor,
        derived_primary_idx: paddle.Tensor,
        derived_sign: paddle.Tensor,
    ):
        super().__init__()
        self.lmin = lmin
        self.lmax = lmax
...
class WignerDataModule(paddle.nn.Module):
    """
    ...
    @property
    def U_blocks(self) -> list[tuple[paddle.Tensor, paddle.Tensor]]:
        """
...
def create_wigner_data_module(lmax: int, lmin: int = 5) -> WignerDataModule:
    """
    ...
    return WignerDataModule(
        coeffs=coeffs, U_blocks=U_blocks, custom_kernels=CustomKernelModule()
    )


def _factorial_table(
    n: int, dtype: paddle.dtype, device: paddle.device
) -> paddle.Tensor:
    """
    Compute factorial table [0!, 1!, 2!, ..., n!].
    """
    table = paddle.zeros(n + 1, dtype=dtype, device=device)
    table[0] = 1.0
...
def _binomial(n: int, k: int, factorial: paddle.Tensor) -> float:
    """
...
def _allocate_case_coeffs(
    n_primary: int, max_poly_len: int, dtype: paddle.dtype, device: paddle.device
) -> CaseCoeffs:
    """
    Allocate tensors for one case (Case1 or Case2).
    """
    return CaseCoeffs(
        coeff=paddle.zeros(n_primary, dtype=dtype, device=device),
        horner=paddle.zeros(n_primary, max_poly_len, dtype=dtype, device=device),
        poly_len=paddle.zeros(n_primary, dtype=paddle.int64, device=device),
        ra_exp=paddle.zeros(n_primary, dtype=dtype, device=device),
        rb_exp=paddle.zeros(n_primary, dtype=dtype, device=device),
        sign=paddle.zeros(n_primary, dtype=dtype, device=device),
    )
...
def _vectorized_horner(
    ...
    device = ratio.device
    dtype = ratio.dtype
    result = paddle.ones(N, n_elements, dtype=dtype, device=device)
    ...
    for i in range(max_poly_len - 1):
        ...
        result = paddle.where(mask.unsqueeze(0), new_result, result)
    return result


def _compute_case_magnitude(
    log_ra: paddle.Tensor,
    log_rb: paddle.Tensor,
    ratio: paddle.Tensor,
    case: CaseCoeffs,
    max_poly_len: int,
) -> paddle.Tensor:
    ...
    ra_powers = paddle.exp(paddle.outer(log_ra, case.ra_exp))
    rb_powers = paddle.exp(paddle.outer(log_rb, case.rb_exp))
    magnitude = case.sign * case.coeff * ra_powers * rb_powers
    return magnitude * horner_sum


def _scatter_primary_to_matrix(
    result: paddle.Tensor, D: paddle.Tensor, coeffs: WignerCoefficients
) -> None:
    """
    ...
    batch_indices = (
        paddle.arange(N, device=device).unsqueeze(1).expand(N, coeffs.n_primary)
    )
...
def _build_euler_transform(ell: int, Jd: paddle.Tensor) -> paddle.Tensor:
    """
    ...
    """
    size = 2 * ell + 1
    U = paddle.zeros(size, size, dtype=Jd.dtype, device=Jd.device)
    for i in range(size):
        m = i - ell
        abs_m = abs(m)
        if abs_m % 2 == 1:
...
def _build_u_matrix(
    ell: int,
    dtype: paddle.dtype = paddle.complex128,
    ...
    U = paddle.zeros(size, size, dtype=dtype, device=device)
    for m in range(-ell, ell + 1):
        ...
        if m > 0:
            ...
            col_neg = -abs_m + ell
            sign = (-1) ** abs_m
            U[row, col_neg] = 1.0j * sqrt2_inv
            U[row, col_pos] = -sign * 1.0j * sqrt2_inv
    return U
...
def precompute_wigner_coefficients(
    lmax: int,
    dtype: paddle.dtype = paddle.float64,
    device: (paddle.device | None) = None,
    lmin: int = 0,
) -> WignerCoefficients:
    """
    ...
    primary_row = paddle.zeros(n_primary, dtype=paddle.int64, device=device)
    primary_col = paddle.zeros(n_primary, dtype=paddle.int64, device=device)
    mp_plus_m = paddle.zeros(n_primary, dtype=dtype, device=device)
    m_minus_mp = paddle.zeros(n_primary, dtype=dtype, device=device)
    diagonal_mask = paddle.zeros(n_primary, dtype=paddle.bool, device=device)
    anti_diagonal_mask = paddle.zeros(n_primary, dtype=paddle.bool, device=device)
    special_2m = paddle.zeros(n_primary, dtype=dtype, device=device)
    anti_diag_sign = paddle.zeros(n_primary, dtype=dtype, device=device)
    ...
    derived_row = paddle.zeros(n_derived, dtype=paddle.int64, device=device)
    derived_col = paddle.zeros(n_derived, dtype=paddle.int64, device=device)
    derived_primary_idx = paddle.zeros(n_derived, dtype=paddle.int64, device=device)
    derived_sign = paddle.zeros(n_derived, dtype=dtype, device=device)
    primary_map = {}
    primary_idx = 0
    block_start = 0
    for ell in range(lmin, lmax + 1):
...
def wigner_d_matrix_real(
    ...
    ra_re = ra_re.to(paddle.float64)
    ra_im = ra_im.to(paddle.float64)
    rb_re = rb_re.to(paddle.float64)
    rb_im = rb_im.to(paddle.float64)
    dtype = paddle.float64
    eps = paddle.finfo(dtype).eps
    eps_sq = eps * eps
    ra_sq = ra_re * ra_re + ra_im * ra_im
    rb_sq = rb_re * rb_re + rb_im * rb_im
    ra_small = ra_sq <= eps_sq
    rb_small = rb_sq <= eps_sq
    ra = paddle.sqrt(paddle.clamp(ra_sq, min=eps_sq))
    rb = paddle.sqrt(paddle.clamp(rb_sq, min=eps_sq))
    general_mask = ~ra_small & ~rb_small
    use_case1 = (ra >= rb) & general_mask
    use_case2 = (ra < rb) & general_mask
    safe_ra_re_phi = paddle.where(ra_small, paddle.ones_like(ra_re), ra_re)
    safe_ra_im_phi = paddle.where(ra_small, paddle.zeros_like(ra_im), ra_im)
    phia = paddle.atan2(x=safe_ra_im_phi, y=safe_ra_re_phi)
    safe_rb_re_phi = paddle.where(rb_small, paddle.ones_like(rb_re), rb_re)
    safe_rb_im_phi = paddle.where(rb_small, paddle.zeros_like(rb_im), rb_im)
    phib = paddle.atan2(x=safe_rb_im_phi, y=safe_rb_re_phi)
    phase = paddle.outer(phia, coeffs.mp_plus_m) + paddle.outer(phib, coeffs.m_minus_mp)
    exp_phase_re = paddle.cos(phase)
    exp_phase_im = paddle.sin(phase)
    safe_ra = paddle.clamp(ra, min=eps)
    safe_rb = paddle.clamp(rb, min=eps)
    log_ra = paddle.log(safe_ra)
    log_rb = paddle.log(safe_rb)
    result_re = paddle.zeros(N, coeffs.n_primary, dtype=dtype, device=device)
    result_im = paddle.zeros(N, coeffs.n_primary, dtype=dtype, device=device)
    arg_rb = phib
    log_mag_rb_power = paddle.outer(log_rb, coeffs.special_2m)
    rb_power_mag = paddle.exp(log_mag_rb_power)
    rb_power_phase = paddle.outer(arg_rb, coeffs.special_2m)
    rb_power_re = rb_power_mag * paddle.cos(rb_power_phase)
    rb_power_im = rb_power_mag * paddle.sin(rb_power_phase)
    ...
    result_re = paddle.where(mask_antidiag, special_val_antidiag_re, result_re)
    result_im = paddle.where(mask_antidiag, special_val_antidiag_im, result_im)
    arg_ra = phia
    log_mag_ra_power = paddle.outer(log_ra, coeffs.special_2m)
    ra_power_mag = paddle.exp(log_mag_ra_power)
    ra_power_phase = paddle.outer(arg_ra, coeffs.special_2m)
    ra_power_re = ra_power_mag * paddle.cos(ra_power_phase)
    ra_power_im = ra_power_mag * paddle.sin(ra_power_phase)
    mask_diag = (rb_small & ~ra_small).unsqueeze(1) & coeffs.diagonal_mask.unsqueeze(0)
    result_re = paddle.where(mask_diag, ra_power_re, result_re)
    result_im = paddle.where(mask_diag, ra_power_im, result_im)
    ...
    result_re = paddle.where(mask1, val1_re, result_re)
    result_im = paddle.where(mask1, val1_im, result_im)
    ...
    result_re = paddle.where(mask2, val2_re, result_re)
    result_im = paddle.where(mask2, val2_im, result_im)
    D_re = paddle.zeros(N, coeffs.size, coeffs.size, dtype=dtype, device=device)
    D_im = paddle.zeros(N, coeffs.size, coeffs.size, dtype=dtype, device=device)
    ...
    if coeffs.n_derived > 0:
        ...
        batch_indices_d = (
            paddle.arange(N, device=device).unsqueeze(1).expand(N, coeffs.n_derived)
        )
...
def _precompute_U_blocks_euler_aligned(
    ...
    if dtype == paddle.float32:
        complex_dtype = paddle.complex64
    else:
        complex_dtype = paddle.complex128
    P = paddle.tensor(
...
def precompute_U_blocks_euler_aligned_real(
    ...
    return [
        (U.real().to(dtype=dtype), U.imag().to(dtype=dtype)) for U in U_blocks_complex
    ]
...
def wigner_d_pair_to_real(
    D_re: paddle.Tensor,
    D_im: paddle.Tensor,
    U_blocks_real: list[tuple[paddle.Tensor, paddle.Tensor]],
    lmax: int,
    lmin: int = 0,
) -> paddle.Tensor:
    """
    ...
    device = D_re.device
    dtype = D_re.dtype
    D_real = paddle.zeros(N, size, size, dtype=dtype, device=device)
    block_start = 0
    for idx, ell in enumerate(range(lmin, lmax + 1)):
        ...
        U_re_T = U_re.T
        U_im_T = U_im.T
        temp_re = paddle.matmul(D_block_re, U_re_T) + paddle.matmul(D_block_im, U_im_T)
        temp_im = paddle.matmul(D_block_im, U_re_T) - paddle.matmul(D_block_re, U_im_T)
        result_re = paddle.matmul(U_re, temp_re) - paddle.matmul(U_im, temp_im)

...
def _load_and_decompress_coefficients() -> dict[str, paddle.Tensor]:
    """
    ...
    """
    raw = paddle.load(path=str(_COEFFICIENTS_FILE))
    result = {}
    for ell in [2, 3, 4]:
        key = f"C_l{ell}"
...
class CustomKernelModule(paddle.nn.Module):
    """
    ...
    def __init__(self):
        ...
        self.register_buffer(
            "C_combined_l3l4", self._build_combined_l3l4(), persistent=False
        )

    def _build_combined_l3l4(self) -> paddle.Tensor:
        """
        ...
        n_mono8 = len(self.monomials_l4)
        C_l3_lifted = paddle.zeros(
        ...
        return paddle.cat([C_l3_lifted, C_l4], dim=0)


def quaternion_to_rotation_matrix(q: paddle.Tensor) -> paddle.Tensor:
    """
    ...
    R = paddle.stack(
        [
            paddle.stack([1 - 2 * (y2 + z2), 2 * (xy - wz), 2 * (xz + wy)], dim=-1),
            paddle.stack([2 * (xy + wz), 1 - 2 * (x2 + z2), 2 * (yz - wx)], dim=-1),
            paddle.stack([2 * (xz - wy), 2 * (yz + wx), 1 - 2 * (x2 + y2)], dim=-1),
        ],
        dim=-2,
    )
    return R
...
def quaternion_to_wigner_d_l2_einsum(
    q: paddle.Tensor, C_l2: paddle.Tensor
) -> paddle.Tensor:
    """
    ...
    D = paddle.einsum("nabcd,ijabcd->nij", q4, C)
    return D
...
def _precompute_powers(
    ...
    z: paddle.Tensor,
    max_power: int,
) -> dict[int, dict[int, paddle.Tensor]]:
    """
    ...
    def powers_for_var(var: paddle.Tensor) -> dict[int, paddle.Tensor]:
        p: dict[int, paddle.Tensor] = {(0): paddle.ones_like(var), (1): var}
...
def quaternion_to_wigner_d_matmul(
    q: paddle.Tensor, ell: int, C: paddle.Tensor, monomials: list[tuple[int, ...]]
) -> paddle.Tensor:
    """
...
def quaternion_to_wigner_d_l3l4_batched(
    q: paddle.Tensor, C_combined: paddle.Tensor, monomials_l4: list[tuple[int, ...]]

...
class SO2_m_Conv(paddle.nn.Module):
    """
    ...
    def __init__(
        self, m: int, sphere_channels: int, m_output_channels: int, lmax: int, mmax: int
    ) -> None:
        super().__init__()
        self.m = m
        ...
        self.fc = paddle.nn.Linear(
            num_channels, 2 * self.out_channels_half, bias=False
        )
        self.fc.weight.data.mul_(1 / math.sqrt(2))

    def forward(self, x_m: paddle.Tensor) -> tuple[paddle.Tensor, paddle.Tensor]:
        x_m = self.fc(x_m)
...
class SO2_m_Conv_Block(paddle.nn.Module):
    """
    ...
    def _build_w_block(self) -> None:
        ...
        self._w_block = paddle.cat(
            [paddle.cat([W1, -W2], dim=1), paddle.cat([W2, W1], dim=1)], dim=0
        ).contiguous()
...
class SO2_Conv1_WithRadialBlock(paddle.nn.Module):
    """
    ...
    def __init__(
        ...
        self.fc_m0 = paddle.nn.Linear(num_channels_m0, m0_output_channels)
        num_channels_rad = self.fc_m0.in_features
        self.so2_m_conv = paddle.nn.ModuleList()
        for m in range(1, mmax + 1):
            self.so2_m_conv.append(
                SO2_m_Conv_Block(m, sphere_channels, m_output_channels, lmax, mmax)
            )
        ...
        self.m_split_sizes = [mappingReduced.m_size[0]] + (
            paddle.tensor(mappingReduced.m_size[1:]) * 2
        ).tolist()
    ...
    def forward(
        self, x: paddle.Tensor, x_edge: paddle.Tensor
        ...
        for m in range(1, self.mmax + 1):
            ...
            out.extend(x_m)
        return paddle.cat(out, dim=1), x_0_extra


class SO2_Conv2_InternalBlock(paddle.nn.Module):
    """
    ...
    def __init__(
        ...
        self.mappingReduced = mappingReduced
        num_channels_m0 = (lmax + 1) * sphere_channels
        self.fc_m0 = paddle.nn.Linear(
            num_channels_m0, m_output_channels * (lmax + 1)
        )
        self.so2_m_conv = paddle.nn.ModuleList()
        for m in range(1, mmax + 1):
            self.so2_m_conv.append(
    ...
    def forward(
        self, x: paddle.Tensor, x_edge: (paddle.Tensor | None) = None
    ) -> paddle.Tensor:
        """
...
def convert_so2_conv1(old: SO2_Convolution) -> SO2_Conv1_WithRadialBlock:
    """
    ...
    """
    assert (
        old.rad_func is not None
    ), "convert_so2_conv1 requires an SO2_Convolution with a RadialMLP (internal_weights=False). Got an SO2_Convolution without rad_func, which means it was created with internal_weights=True."
...
def convert_so2_conv2(old: SO2_Convolution) -> SO2_Conv2_InternalBlock:
    """
...
class SO2_Convolution(paddle.nn.Module):
    """
    ...
    def __init__(
        ...
        self.m_split_sizes = [self.mappingReduced.m_size[0]] + (
            paddle.tensor(self.mappingReduced.m_size[1:]) * 2
        ).tolist()
        self.edge_split_sizes = [self.fc_m0.in_features] + [
            mod.fc.in_features for mod in self.so2_m_conv
        ]
...
class SO2_Linear(paddle.nn.Module):
    """
    ...
    def __init__(
        ...
        num_channels_rad = self.fc_m0.in_features
        self.so2_m_fc = paddle.nn.ModuleList()
        for m in range(1, self.mmax + 1):
            num_coefficents = self.lmax - m + 1
    ...
    def forward(self, x: paddle.Tensor, x_edge: paddle.Tensor) -> paddle.Tensor:
        batch_size = x.shape[0]
        out = []
        x = paddle.einsum("nac,ba->nbc", x, self.mappingReduced.to_m)
        if self.rad_func is not None:
            x_edge = self.rad_func(x_edge)
        offset_rad = 0
        ...
        out = paddle.cat(out, dim=1)
        out = paddle.einsum("nac,ab->nbc", out, self.mappingReduced.to_m)
        return out

...
def _softmax(x):
    return paddle.softmax(x, dim=1) + 0.005


def _pnorm(x):
    return paddle.nn.functional.normalize(x.abs() + 2 / x.shape[0], p=1.0, dim=1)
...
@dataclass
class MOLEGlobals:
    expert_mixing_coefficients: paddle.Tensor
    mole_sizes: paddle.Tensor
    ac_start_idx: int = 0


def init_linear(num_experts, use_bias, out_features, in_features):
    k = math.sqrt(1.0 / in_features)
    weights = paddle.nn.Parameter(
        k * 2 * (paddle.rand(num_experts, out_features, in_features) - 0.5)
    )
    bias = (
        paddle.nn.Parameter(k * 2 * (paddle.rand(out_features) - 0.5))
        if use_bias
        else None
    )
    return weights, bias
...
class MOLEDGL(paddle.nn.Module):
    def __init__(
        self, num_experts, in_features, out_features, global_mole_tensors, bias: bool
    ):
        super().__init__()
    ...
    def forward(self, x):
        with paddle.autocast(device_type=self.weights.device.type, enabled=False):
            weights = paddle.einsum(
                "eoi, be->bio",
                self.weights,
...
class MOLE(paddle.nn.Module):
    ...
    def merged_linear_layer(self):
        linear = paddle.compat.nn.Linear(
            ...
            if interval_overlap is not None:
                ...
                out.append(
                    paddle.compat.nn.functional.linear(
                        x[start:end], weights[n], bias=self.bias
                    )
                )
        result = paddle.concatenate(out, dim=0)
        assert (

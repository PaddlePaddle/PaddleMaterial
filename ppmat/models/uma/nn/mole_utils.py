...
from .mole import MOLE, MOLEDGL, MOLEGlobals, norm_str_to_fn
from .so2_layers import SO2_Convolution
...
def recursive_replace_so2m0_linear(model, replacement_factory):
    for _, child in model.named_children():
        if isinstance(child, paddle.nn.Module):
...
def recursive_replace_all_linear(model, replacement_factory):
    for child_name, child in model.named_children():
        ...
        elif isinstance(child, paddle.nn.Module):
...
def replace_linear_with_shared_linear(existing_linear_module, cache):
    layer_identifier = (
...
def convert_model_to_MOLE_model(
    ...
    model.routing_mlp = paddle.nn.Sequential(
        paddle.nn.Linear(routing_mlp_dim, num_experts * 2, bias=True),
        paddle.nn.SiLU(),
        paddle.nn.Linear(num_experts * 2, num_experts * 2, bias=True),
        paddle.nn.SiLU(),
        paddle.nn.Linear(num_experts * 2, num_experts, bias=True),
        paddle.nn.SiLU(),
    )

...
from ppmat.models.common.e3nn.o3 import Irreps


def _transform(
    dat, irreps_dat, rot_mat, translation=0.0, output_transform_dtype: bool = False
):
    """Transform ``dat`` by ``rot_mat`` and ``translation`` according to ``irreps_dat``."""
    out = []
    transform_dtype = rot_mat.dtype
    translation = paddle.to_tensor(translation, dtype=transform_dtype)
    for irreps, a in zip(irreps_dat, dat):
        ...
        if irreps is None:
            ...
            translation = paddle.to_tensor(translation, place=a.place)
            out.append(
                (
                    a.astype(transform_dtype) @ rot_mat.T.astype(a.dtype) + translation
                ).astype(out_dtype)
            )
        else:
            out.append(
                (
                    a.astype(transform_dtype)
                    @ irreps.D_from_matrix(rot_mat).T.astype(a.dtype)
                ).astype(out_dtype)
            )
    return out
...
def _get_io_irreps(func, irreps_in=None, irreps_out=None):
    ...
    if (irreps_in is None or irreps_out is None) and isinstance(func, paddle.nn.Layer):
        warnings.warn(
            "Asking to infer irreps in/out of a module. Please provide `irreps_in` "
...
def _rand_args(irreps_in, batch_size: Optional[int] = None):
    ...
    args_in = [
        (
            paddle.randn([batch_size, 3])
            if (irreps == "cartesian_points")
            else irreps.randn(batch_size, -1)
        )
...
def _get_device(mod: paddle.nn.Layer) -> str:
    for param in mod.parameters():
        return param.place._str()
    for buffer in mod.buffers():
        return buffer.place._str()
    return "cpu"


def _get_floating_dtype(mod: paddle.nn.Layer) -> str:
    for param in mod.parameters():
        if param.dtype in ["float32", "float64"]:
            return param.dtype
    for buffer in mod.buffers():
        if buffer.dtype in ["float32", "float64"]:
            return buffer.dtype
...
def _to_device_dtype(args, device=None, dtype=None):
    ...
    if isinstance(args, paddle.Tensor):
        if args.dtype in ["float32", "float64"]:
            return (
                args.astype(dtype=dtype).cuda()
                if device == "gpu"
                else args.astype(dtype=dtype)
            )
        else:
            return args.cuda() if device == "gpu" else args
        ...
        raise TypeError(
            "Only (nested) dict/tuple/lists of Tensors can be moved to a device/dtype."
        )

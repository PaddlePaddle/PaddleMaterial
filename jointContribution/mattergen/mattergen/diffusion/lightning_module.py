...
class OptimizerPartial(Protocol):
    ...
    def __call__(self, params: Any) -> paddle.optimizer.Optimizer:
...
class SchedulerPartial(Protocol):
    ...
    def __call__(self, optimizer: paddle.optimizer.Optimizer) -> Any:
        raise NotImplementedError


def get_default_optimizer(params):
>>>>>>    return torch.optim.AdamW(params=params, lr=0.0001, weight_decay=0, amsgrad=True)


>>>>>>class DiffusionLightningModule(pytorch_lightning.LightningModule, Generic[T]):
    ...
    def __init__(
        ...
        scheduler_partials: Optional[
            Sequence[Dict[str, Union[Any, SchedulerPartial]]]
        ] = None,
    ):
        """_summary_
    ...
    def load_from_checkpoint(
        cls, checkpoint_path: str, map_location: Optional[str] = None, **kwargs
        ...
        lightning_module.set_state_dict(state_dict=checkpoint["state_dict"])
    ...
    def load_from_checkpoint_and_config(
        ...
        assert isinstance(lightning_module, cls)
        result = lightning_module.set_state_dict(state_dict=checkpoint["state_dict"])
    ...
    def training_step(
        self, train_batch: T, batch_idx: int
    ) -> pytorch_lightning.utilities.types.STEP_OUTPUT:
        return self._calc_loss(train_batch, True)

    def validation_step(
        self, val_batch: T, batch_idx: int
    ) -> Optional[pytorch_lightning.utilities.types.STEP_OUTPUT]:
        return self._calc_loss(val_batch, False)

    def test_step(
        self, test_batch: T, batch_idx: int
    ) -> Optional[pytorch_lightning.utilities.types.STEP_OUTPUT]:
        return self._calc_loss(test_batch, False)

    def _calc_loss(
        self, batch: T, train: bool
    ) -> Optional[pytorch_lightning.utilities.types.STEP_OUTPUT]:

...
python -m paddle.distributed.launch --gpus="0,1,2,3" training_structure_generation/train.py --config-name task_diffcsp/diffcsp_mp20.yaml
# single-gpu training
python training_structure_generation/train.py --config-name task_diffcsp/diffcsp_mp20.yaml
```
...
python training_structure_generation/train.py --config-name task_diffcsp/diffcsp_mp20.yaml Global.do_eval=True Global.do_train=False Global.do_test=False Trainer.pretrained_model_path='your model path(*.pdparams)'
```

### Testing
```bash
# This command is used to evaluate the model's performance on the test dataset.
python training_structure_generation/train.py --config-name task_diffcsp/diffcsp_mp20.yaml Global.do_eval=False Global.do_train=False Global.do_test=True Trainer.pretrained_model_path='your model path(*.pdparams)'
```

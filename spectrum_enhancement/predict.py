...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Spectrum enhancement prediction")
    ...
    parser.add_argument(
        "--device",
        help="Paddle device, for example cpu or gpu:0.",
    )
...
def main():
    ...
    if save_path is None:
        ...
        if trainer_output_dir:
            save_path = osp.join(trainer_output_dir, "predictions")
        elif args.config_path:
            save_path = osp.join("./output", Path(args.config_path).stem, "predictions")
        else:
